...
def _augment_with_fmp(
    ...
    if metrics:
        if data.get("WACC") is None or isinstance(data.get("WACC"), str):
            data["WACC"] = _av_number(metrics.get("wacc"))
        if data.get("Beta") is None:
            data["Beta"] = _av_number(metrics.get("beta"))
    ...
    if balance:
        ...
        if data.get("Current_Assets") is None:
            data["Current_Assets"] = _av_number(balance.get("totalCurrentAssets"))
        if data.get("Current_Liabilities") is None:
            data["Current_Liabilities"] = _av_number(balance.get("totalCurrentLiabilities"))
...
def _prepare_row(
    ...
    data["Total_Debt"] = _extract_field(
        balance_df,
        ["Total Debt", "Long Term Debt", "Short Long Term Debt", "Current Portion of Long Term Debt"],
    )
