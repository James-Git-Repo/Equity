...
def _safe_loc(df: pd.DataFrame, row_name: str) -> Optional[float]:
    ...
    The helper mirrors the common layout of yfinance statements where columns
    represent periods (most recent first). It drops nulls and returns the first
    usable entry, falling back to the last available value when ordering is
    ambiguous. Any parsing errors result in ``None`` so callers can continue to
...
def _yahoo_eps_cagr(financials: pd.DataFrame) -> Optional[float]:
    try:
        row = financials.loc["Diluted EPS"].dropna()
        if row.empty:
            return None
        eps_series = pd.Series(row.values, index=_coerce_year_labels(row.index))
...
def _yahoo_revenue_cagr(financials: pd.DataFrame) -> Optional[float]:
    try:
        ...
        if row.empty:
            return None
        rev_series = pd.Series(row.values, index=_coerce_year_labels(row.index))
...
def _yahoo_fcf_cagr(cashflow_df: pd.DataFrame) -> Optional[float]:
    try:
        ocf_hist = cashflow_df.loc["Total Cash From Operating Activities"].dropna()
        capex_hist = cashflow_df.loc["Capital Expenditures"].dropna()
    except Exception:
        return None
    common_idx = ocf_hist.index.intersection(capex_hist.index)
    if len(common_idx) < 4:
        return None
    fcf_hist = ocf_hist[common_idx] + capex_hist[common_idx]
    fcf_series = pd.Series(fcf_hist.values, index=_coerce_year_labels(fcf_hist.index))
