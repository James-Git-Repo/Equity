...
class PipelineTests(unittest.TestCase):
    def test_build_dataset_populates_from_yahoo(self):
        ...
        self.assertAlmostEqual(row["WACC"], 0.0696, places=3)
    ...
    def test_fmp_third_source_backfills_cashflow_and_wacc(self):
        def fake_ticker(symbol):
            return StubTicker(symbol, {}, pd.DataFrame(), pd.DataFrame(), pd.DataFrame())

        class StubFMP(main.FinancialModelingPrepClient):
            def __init__(self):
                self.api_key = "x"

            def get_key_metrics(self, symbol):
                return {"wacc": "0.08", "beta": "0.9"}

            def get_profile(self, symbol):
                return {"beta": "0.9", "wacc": "0.081"}

            def get_balance_sheet(self, symbol):
                return {"totalCurrentAssets": "100", "totalCurrentLiabilities": "60", "totalDebt": "40"}

            def get_cash_flow(self, symbol):
                return {"netCashProvidedByOperatingActivities": "70", "freeCashFlow": "55"}

        with patch("main.yf.Ticker", side_effect=fake_ticker):
            df = main.build_dataset(["FMPX"], av_client=None, fmp_client=StubFMP())

        row = df.iloc[0]
        self.assertEqual(row["OCF"], 70.0)
        self.assertEqual(row["FCF"], 55.0)
        self.assertEqual(row["Current_Assets"], 100.0)
        self.assertEqual(row["Current_Liabilities"], 60.0)
        self.assertEqual(row["Total_Debt"], 40.0)
        self.assertEqual(row["WACC"], 0.08)

    def test_similar_label_detection_for_current_assets_and_ocf(self):
        income_df = pd.DataFrame({"2023": {"Income Before Tax": 100, "Income Tax Expense": 20}})
        balance_df = pd.DataFrame({"2023": {"Current Assets (Total)": 50, "Current Liability": 25}})
        cashflow_df = pd.DataFrame(
            {"2023": {"Net cash provided by operating activities": 30, "Purchase of property and equipment": -5}}
        )

        def fake_ticker(symbol):
            return StubTicker(symbol, {"currentPrice": 1, "sharesOutstanding": 1_000_000, "beta": 1}, income_df, balance_df, cashflow_df)

        with patch("main.yf.Ticker", side_effect=fake_ticker):
            df = main.build_dataset(["FUZZY"], av_client=None, fmp_client=None)

        row = df.iloc[0]
        self.assertEqual(row["Current_Assets"], 50)
        self.assertEqual(row["Current_Liabilities"], 25)
        self.assertEqual(row["OCF"], 30)
        self.assertEqual(row["FCF"], 25)

    def test_yahoo_statement_augments_growth_and_altman(self):
        income_df = pd.DataFrame(
            {
                "2023": {
                    "Total Revenue": 1200,
        ...
        cashflow_df = pd.DataFrame(
            {
                "2023": {"Total Cash From Operating Activities": 360, "Capital Expenditures": -90},
                "2022": {"Total Cash From Operating Activities": 330, "Capital Expenditures": -80},
                "2021": {"Total Cash From Operating Activities": 300, "Capital Expenditures": -70},
                "2020": {"Total Cash From Operating Activities": 270, "Capital Expenditures": -60},
            }
        )

        dividends = pd.Series(
            data=[0.6, 0.5, 0.4, 0.3],
            index=pd.to_datetime(["2023-04-01", "2022-04-01", "2021-04-01", "2020-04-01"]),
        )

        stub_info = {"currentPrice": 50, "sharesOutstanding": 200_000, "beta": 1.1}

        def fake_ticker(symbol):
            return StubTicker(symbol, stub_info, income_df, balance_df, cashflow_df, dividends=dividends)

        with patch("main.yf.Ticker", side_effect=fake_ticker):
            df = main.build_dataset(["GROW"], av_client=None, fmp_client=None)

        row = df.iloc[0]
        self.assertIsInstance(row["EPS_CAGR_3Y"], float)
        self.assertGreater(row["EPS_CAGR_3Y"], 0)
        self.assertIsInstance(row["Altman_Z"], float)
        self.assertAlmostEqual(row["Tax_Rate"], 0.2)
