...
- When Yahoo Finance data is unavailable, the tool will attempt to backfill key
  ...
  Fuzzy label matching is applied so renamed fields like "Current Assets"
  versus "Current Assets (Total)" are still captured. When all providers lack
  WACC, the exporter estimates one from beta (CAPM) and capital structure and
  falls back to a conservative baseline if no inputs exist.
- Yahoo statement data is inspected directly (beyond the `info` dictionary) to
  recover metrics with shifting labels—especially OCF, FCF, current assets/
  liabilities, and interest expense—and to derive growth rates (EPS/FCF/
  revenue/dividend CAGRs), effective tax rate, and Altman Z when raw
...
share, insider activity, and analyst ratings beyond Yahoo) are not supplied by
any of the sources and will remain blank. Altman Z, WACC, and tax rate are
computed from statements when inputs exist and otherwise fall back to external
providers or conservative estimates.
